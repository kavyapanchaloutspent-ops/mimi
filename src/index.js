...
let loveData = { users: {}, relationships: {}, archives: {}, casinos: {} };
try {
  const loaded = JSON.parse(readFileSync(LOVE_DATA_FILE, "utf8"));
  loveData = { users: loaded.users || {}, relationships: loaded.relationships || {}, archives: loaded.archives || {}, casinos: loaded.casinos || {} };
} catch {}
...
async function initializePersistence() {
  ...
  if (result.rows[0]?.data) {
    ...
    loveData = {
      ...
      casinos: stored.loveData?.casinos || {},
    };
...
async function findSoulmate(message) {
  ...
  if (!candidates.length) return message.reply({ embeds: [errorEmbed("Server chưa có người phù hợp để tìm duyên.")] });
  const best = candidates.slice(0, 10);
  const perfect = candidates.filter((item) => item.score === 100);
  const ranking = best.map((item, index) => `**${index + 1}.** ${item.user} — **${item.score}%**`).join("\n");
  const verdict = perfect.length
    ? `\n\nTìm thấy **${perfect.length} người đạt 100%**: ${perfect.slice(0, 5).map((item) => `${item.user}`).join(", ")}`
    : `\n\nChưa có ai đạt 100%. Người hợp nhất hiện tại là ${best[0].user} với **${best[0].score}%**.`;
  const embed = baseEmbed("Máy tìm duyên", `Kết quả dành cho ${subject}:\n\n${ranking}${verdict}`, perfect.length ? 0xff2e63 : 0xff6fae)
    .setThumbnail(subject.displayAvatarURL({ size: 256 }))
    .setFooter({ text: `Đã quét ${candidates.length} thành viên thật · điểm cố định theo cặp Discord ID` });
...
function casinoTier(wagered = 0) {
  if (wagered >= 10_000_000) return { name: "DIAMOND", icon: "💎", daily: 200 };
  if (wagered >= 2_000_000) return { name: "PLATINUM", icon: "🏆", daily: 120 };
  if (wagered >= 500_000) return { name: "GOLD", icon: "🥇", daily: 70 };
  if (wagered >= 100_000) return { name: "SILVER", icon: "🥈", daily: 30 };
  return { name: "BRONZE", icon: "🥉", daily: 0 };
}
...
async function sendCasinoDaily(message) {
  ...
  const tier = casinoTier(user.casino.wagered);
  const reward = 180 + Math.floor(Math.random() * 121) + user.txStreak * 20 + tier.daily;
  user.coins = (user.coins || 0) + reward;
  user.lastDailyTx = today;
  saveLoveData();
  return message.reply({ embeds: [baseEmbed("Lộc casino hằng ngày", `${message.author} nhận **${reward} coin**.\nChuỗi điểm danh: **${user.txStreak}/7 ngày** · VIP **${tier.name}** +${tier.daily} · Số dư: **${user.coins} coin**`, 0xf4a261)] });
}

async function sendBalance(message) {
  const user = casinoUser(message.author.id), c = user.casino;
  const tier = casinoTier(c.wagered);
  return message.reply({ embeds: [baseEmbed(`Ví casino · ${tier.icon} ${tier.name}`, `${message.author} đang có **${user.coins || 0} coin**.`, 0xf4a261).addFields(
...
function taiXiuHistory(guildId) {
  loveData.casinos ||= {};
  loveData.casinos[guildId] ||= { taiXiu: [] };
  loveData.casinos[guildId].taiXiu ||= [];
  return loveData.casinos[guildId].taiXiu;
}

function addTaiXiuResult(guildId, entry) {
  const history = taiXiuHistory(guildId);
  history.push(entry);
  loveData.casinos[guildId].taiXiu = history.slice(-60);
  saveLoveData();
}

function analyzeTaiXiu(history) {
  const recent = history.slice(-20);
  const tai = recent.filter(x => x.side === "tai").length;
  const xiu = recent.filter(x => x.side === "xiu").length;
  let currentStreak = 0, streakSide = recent.at(-1)?.side || null;
  for (let i = recent.length - 1; i >= 0 && recent[i].side === streakSide; i -= 1) currentStreak += 1;
  const suggestion = !recent.length ? "CHƯA ĐỦ DỮ LIỆU" : tai - xiu >= 4 ? "CẦU ĐANG NGHIÊNG TÀI" : xiu - tai >= 4 ? "CẦU ĐANG NGHIÊNG XỈU" : currentStreak >= 3 ? `BỆT ${streakSide.toUpperCase()} ${currentStreak}` : "CẦU CÂN — NÊN QUAN SÁT";
  return { recent, tai, xiu, currentStreak, streakSide, suggestion };
}
...
function renderTaiXiuBoard(guildName, history) {
  const canvas = createCanvas(1200, 675), ctx = canvas.getContext("2d");
  const gradient = ctx.createLinearGradient(0, 0, 1200, 675); gradient.addColorStop(0, "#080b16"); gradient.addColorStop(1, "#17102c"); ctx.fillStyle = gradient; ctx.fillRect(0, 0, 1200, 675);
  ctx.fillStyle = "#f5c76b"; ctx.font = "bold 44px sans-serif"; ctx.fillText("VIP TÀI XỈU — SOI CẦU", 55, 72);
  ctx.fillStyle = "#a9b0c3"; ctx.font = "22px sans-serif"; ctx.fillText(String(guildName || "Discord Casino").slice(0, 55), 57, 108);
  const a = analyzeTaiXiu(history);
  roundRect(ctx, 55, 140, 1090, 355, 24, "#0f1424");
  const shown = history.slice(-30);
  shown.forEach((item, i) => { const col=i%15,row=Math.floor(i/15),x=95+col*69,y=190+row*135; ctx.beginPath();ctx.arc(x,y,25,0,Math.PI*2);ctx.fillStyle=item.side==="tai"?"#f0b429":item.side==="xiu"?"#4aa8ff":"#e84a5f";ctx.fill();ctx.fillStyle="#07101d";ctx.font="bold 17px sans-serif";ctx.textAlign="center";ctx.fillText(String(item.total),x,y+6);ctx.fillStyle="#dfe6f3";ctx.font="14px sans-serif";ctx.fillText(item.side.toUpperCase(),x,y+49); });
  ctx.textAlign="left"; roundRect(ctx,55,525,1090,105,20,"#121a2d");ctx.font="bold 25px sans-serif";ctx.fillStyle="#f0b429";ctx.fillText(`TÀI ${a.tai}`,85,570);ctx.fillStyle="#4aa8ff";ctx.fillText(`XỈU ${a.xiu}`,225,570);ctx.fillStyle="#e8ecf5";ctx.fillText(a.suggestion,390,570);ctx.font="16px sans-serif";ctx.fillStyle="#8d96aa";ctx.fillText("Thống kê tham khảo từ lịch sử — mỗi ván vẫn ngẫu nhiên, không bảo đảm thắng.",85,607);
  return canvas.toBuffer("image/png");
}

async function sendTaiXiuBoard(message) {
  const history = taiXiuHistory(message.guild.id);
  const image = renderTaiXiuBoard(message.guild.name, history);
  return message.reply({ embeds: [baseEmbed("Soi cầu tài xỉu VIP", history.length ? `Đã phân tích **${Math.min(20, history.length)}** ván gần nhất.` : "Chưa có ván nào; chơi `.tx` để tạo lịch sử.", 0xf0b429).setImage("attachment://cau-tai-xiu.png")], files: [new AttachmentBuilder(image, { name: "cau-tai-xiu.png" })] });
}
...
async function playTaiXiu(message, args) {
  ...
  const animation = await message.reply({ embeds: [baseEmbed("VIP Tài xỉu", `Đã nhận cược **${wager.bet} coin** cửa **${choice.toUpperCase()}**.\n\n🎲 Đang lắc bát...`, 0xf0b429)] });
  for (let frame = 1; frame <= 3; frame += 1) {
    await wait(420);
    const fake = [1,2,3].map(() => 1 + Math.floor(Math.random() * 6));
    await animation.edit({ embeds: [baseEmbed("VIP Tài xỉu", `Đang mở bát${".".repeat(frame)}\n\n🎲 **${fake.join(" · ")}**`, 0xf0b429)] }).catch(() => {});
  }
  ...
  addTaiXiuResult(message.guild.id, { dice, total, side: triple ? "bao" : side, triple, at: new Date().toISOString() });
  const image = renderTaiXiuBoard(message.guild.name, taiXiuHistory(message.guild.id));
  const embed = casinoResultEmbed("VIP Tài xỉu — Kết quả", result, `🎲 **${dice.join(" · ")}** = **${total}** — ${triple ? "BỘ BA, NHÀ CÁI ĂN" : side.toUpperCase()}`).setImage("attachment://cau-tai-xiu.png");
  return animation.edit({ embeds: [embed], files: [new AttachmentBuilder(image, { name: "cau-tai-xiu.png" })] });
}
...
function casinoHelpEmbed() { return baseEmbed("Cờ bạc — Danh sách lệnh", "Mọi trò dùng chung coin với `.daily` và cửa hàng tình yêu.", 0xf4a261).addFields(
  ...
  { name:"Tài xỉu VIP", value:"`.tx tai 100` · `.tx xiu 100` · `.soicau` — animation + ảnh cầu" },
...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    if (command === "soicau" || command === "cau") return sendTaiXiuBoard(message);
